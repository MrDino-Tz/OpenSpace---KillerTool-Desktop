...
export default function PdfEditor() {
  ...
  const handleFileUpload = async (e) => {
    if (e?.target?.files?.[0]) { loadPdf(e.target.files[0]); return; }
    const result = await pickAndReadFile({ filters: [{ name: 'PDF', extensions: ['pdf'] }] });
    if (result) loadPdf(new File([result.data], result.name));
  };
  ...
  const handleMergeFile = async (e) => {
    ...
    try {
      ...
      const name = pdfFile.name.replace('.pdf', '_merged.pdf');
      const saved = await saveAndWriteFile(name, mergedBytes, 'application/pdf');
      if (saved) showSnack('Merged PDF saved');
      else browserDownloadFile(mergedBytes, name, 'application/pdf');
  ...
  const handleSplit = async () => {
    ...
    try {
      ...
      const name = pdfFile.name.replace('.pdf', '_split.pdf');
      const saved = await saveAndWriteFile(name, bytes, 'application/pdf');
      if (saved) showSnack('Split PDF saved');
      else browserDownloadFile(bytes, name, 'application/pdf');
  ...
  const downloadModifiedPdf = async () => {
    ...
    try {
      ...
      const name = pdfFile.name.replace('.pdf', '_edited.pdf');
      const saved = await saveAndWriteFile(name, bytes, 'application/pdf');
      if (saved) showSnack('PDF saved successfully');
      else browserDownloadFile(bytes, name, 'application/pdf');
    ...
    <Box sx={{ width: '100%', maxWidth: '100%', overflowX: 'hidden' }}>
          ...
          <MainCard title={pdfFile ? `PDF Editor — ${pdfFile.name}` : 'PDF Editor'} sx={{ '& .MuiCardContent-root': { p: { xs: 2, md: 3 } } }}>
            <Stack spacing={3}>
              ...
              <Stack direction="row" spacing={1.5} alignItems="center" flexWrap="wrap" useFlexGap>
                <Button variant="contained" startIcon={<CloudUploadOutlined />} size="small" disabled={loading} onClick={() => handleFileUpload()}>
                    ...
                    <Stack direction="row" spacing={0.75} alignItems="center" flexWrap="wrap" useFlexGap>
                      ...
                      <Tooltip title="Add image">
                        <IconButton size="small" color={tool === 'addImage' ? 'primary' : 'default'}
                          onClick={async () => {
                            setTool('addImage');
                            const result = await pickAndReadFile({ filters: [{ name: 'Images', extensions: ['png', 'jpeg', 'jpg'] }] });
                            if (result) {
      ...
      <Dialog open={mergeOpen} onClose={() => setMergeOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="contained" startIcon={<CloudUploadOutlined />} onClick={async () => { const r = await pickAndReadFile({ filters: [{ name: 'PDF', extensions: ['pdf'] }] }); if (r) { const ev = { target: { files: [new File([r.data], r.name)] } }; handleMergeFile(ev); } }}>Select PDF</Button>
        </DialogActions>
      </Dialog>
